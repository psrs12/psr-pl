...
import (
	...
	"offer-acceptance-service/internal/offeracceptance"
)

func main() {
	logger := slog.New(slog.NewJSONHandler(os.Stdout, nil))

	tableName := requireEnv(logger, "OFFER_ACCEPTANCE_TABLE_NAME")
	...
	repo := offeracceptance.NewDynamoRepository(dynamoClient, tableName)
	appClient := offeracceptance.NewApplicationManagementClient(appManagementBaseURL, nil)
	service := offeracceptance.NewService(repo, appClient)
	handler := offeracceptance.NewHandler(service, logger)
	...
	go func() {
		logger.Info("starting offer-acceptance-service", "port", port)
