...
import (
	...
	"document-service/internal/document"
)

func main() {
	logger := slog.New(slog.NewJSONHandler(os.Stdout, nil))

	tableName := requireEnv(logger, "DOCUMENT_TABLE_NAME")
	...
	repo := document.NewDynamoRepository(dynamoClient, tableName)
	appClient := document.NewApplicationManagementClient(appManagementBaseURL, nil)
	service := document.NewService(repo, appClient)
	handler := document.NewHandler(service, logger)
	...
	server := &http.Server{
		Addr:              ":" + port,
		Handler:           document.WithRequestID(withCORS(mux, envOrDefault("CORS_ALLOWED_ORIGIN", "*"))),
		ReadHeaderTimeout: 5 * time.Second,
	}

	go func() {
		logger.Info("starting document-service", "port", port)
